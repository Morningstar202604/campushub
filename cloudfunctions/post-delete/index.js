...
// 删除帖子：软删除（status='deleted'）+ 归属校验/管理员 + 计数回退 + 回收云存储图片
...
exports.main = wrap(async (event) => {
  ...
  const images = Array.isArray(post.images)
    ? post.images.filter(f => typeof f === 'string' && f.startsWith('cloud://'))
    : []
  if (images.length) {
    cloud.deleteFile({ fileList: images }).catch(e => console.warn('[post-delete] 图片清理失败(已忽略):', e))
  }
