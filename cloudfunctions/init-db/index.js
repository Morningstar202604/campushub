...
exports.main = async (event, context) => {
  ...
  // 1. 创建集合（createCollection 在集合已存在时会报错，忽略即可）
  const collections = [
  ...
  // 2. 导入指南分类
  const categories = [
    { name: '新生入学', sort: 1, schoolId: 'HSFNC', icon: '🎓' },
    { name: '学习攻略', sort: 2, schoolId: 'HSFNC', icon: '📚' },
    { name: '生活指南', sort: 3, schoolId: 'HSFNC', icon: '🌿' },
    { name: '美食地图', sort: 4, schoolId: 'HSFNC', icon: '🍜' },
    { name: '交通出行', sort: 5, schoolId: 'HSFNC', icon: '🚌' },
    { name: '周边玩乐', sort: 6, schoolId: 'HSFNC', icon: '🎮' }
  ]
  ...
  if (existingCats.total === 0) {
    for (const cat of categories) {
      await db.collection('guide_categories').add({
        data: { ...cat, createdAt: new Date() }
      })
    }
