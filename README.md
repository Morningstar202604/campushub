...
- **架构**：`cloudfunctions/common/` 共享内核层（鉴权 / 内容安全 / 封禁 / 限流 / 返回格式单一事实来源）
...
所有云函数的鉴权、内容安全、封禁拦截、频率限制、返回格式都收敛到
`cloudfunctions/common/`。每个云函数用一行引入：

```js
const { getDB, ok, wrap, requireActiveUser, checkContents, rateLimit } = require('./common-bundle')
```
...
在微信云开发控制台给 `admin` 函数配置环境变量后即可调用封禁/解封。
