...
第二轮三线交叉复核与 v0.9.2 修复批次（限频原子化 / 查询面收敛 / 定时函数守卫等 10 项）已并入本仓库，明细见 `docs/REVIEW-20260922.md` 第六节。
...
| 5 | 页面结构 / 致命反模式扫描 | `python uni-app/scripts/verify_pages.py` | 通过 | `uni-app/scripts/verify_pages.out.txt`：page_count=20；31 文件反模式扫描 `real_code_hits=0`；`struct_failures=0`；样式分层（pages_importing_tokens=0）与 14 项关键一致性点 `consistency_failures=0` |
...
| 7 | 编辑功能专项核验 | `python uni-app/scripts/verify_edit.py`（原 `_verify_edit.py`，v0.9.2 转正随仓） | 通过 | `uni-app/scripts/verify_edit.out.txt`：my-list / post-create / product-create / product-detail / adapters 共 5 文件关键改动全部落盘（`missing: NONE`），括号/标签/文本配平全部成对，`forbidden_real_code_hits=0`，`VERDICT=PASS`。说明：输出中两处 `STILL BAD` 为校验脚本对 `form.images.splice(i,1)`（合法数组删除操作）的启发式误报，非真实缺陷，最终判据 `forbidden_real_code_hits=0` 为准 |
...
| L | `verify_edit.py`（随仓转正版，原 `_verify_edit.py`）对 `form.images.splice` 存在启发式误报（输出 STILL BAD 但实为合法代码） | 本报告补充 | 校验脚本白名单该模式，避免误导后续验收人 |

---

## 四、买家复跑验证（可原样执行）

环境要求：Node.js ≥ 18（建议 20/22 LTS）。以下命令在仓库 `uni-app/` 目录下原样执行：

```bash
cd uni-app   # 前端工程目录（仓库根下的 uni-app/）
...
- 第二轮三线交叉复核（前端补盲 / 后端 37 函数全量 / 交付链复验）与 v0.9.2 修复批次全量脚本验证（明细见 `docs/REVIEW-20260922.md` 第六节）
...
- [ ] 按第四节命令在买方环境复跑：type-check / build:h5 / build:mp-weixin / verify_pages / verify_edit 全部通过
- [ ] 按 `docs/DEPLOY.md` 完成 CloudBase 部署（37 函数、HTTP 访问、匿名登录、索引/TTL、init-db、task-expire 触发器；索引清单以 `docs/INDEXES.md` 45 条为准，含 v0.9.2 新增 `idx_rate_limits_expire` / `idx_view_logs_created`）
